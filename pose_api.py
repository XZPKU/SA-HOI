...
def call_pose_api(img_path):
    ...
    #end_time = time.time()
    #print(f'api time is {end_time-st_time}')
    #result = next(result_generator)
    result = [result for result in result_generator]
    return result
def call_3d_pose_api(img_path):
    inferencer = MMPoseInferencer(pose3d='human3d')
    result_generator = inferencer(img_path,shows = False)
    #result = next(result_generator)
    result = [result for result in result_generator]
    return result
def call_wb_pose_api(img_path):
    inferencer = MMPoseInferencer(pose2d='wholebody')
    result_generator = inferencer(img_path,shows = False)
    #result = next(result_generator)
    result = [result for result in result_generator]
    return result
# def call_hand_pose_api(img_path):
...
def call_hand_pose_api(img_path):
    inferencer = MMPoseInferencer(pose2d='hand')
    result_generator = inferencer(img_path,shows = False,vis_out_dir='vis_results',kpt_thr=0,black_background=True)
    #result = next(result_generator)
    result = [result for result in result_generator]
    return result
def main():
